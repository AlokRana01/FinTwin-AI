...
class FinancialSnapshot:
    ...
    def _compute_health_score(self) -> float:
        """
        Simplified 5-component health score matching HealthScoreEngine logic.
        Weights: Savings Rate 25%, EMI Burden 20%, Emergency Fund 20%,
                 Investment Ratio 15%, Debt Level (not fully in snapshot) 20%.
        """
        # Savings Rate Score (target >= 30%)
        sr_score = min(self.savings_rate / 0.30, 1.0) * 100.0

        # EMI Burden Score (target <= 35%)
        if self.emi_ratio <= 0.35:
            emi_score = 100.0
        elif self.emi_ratio >= 0.70:
            emi_score = 0.0
        else:
            emi_score = 100.0 - ((self.emi_ratio - 0.35) / 0.35) * 100.0

        # Emergency Fund Score (target >= 6 months)
        outflow = self.basic_expenses + self.monthly_emi
        if outflow > 0:
            ef_score = min((self.emergency_fund / outflow) / 6.0, 1.0) * 100.0
        else:
            ef_score = 100.0 if self.emergency_fund > 0 else 0.0

        # Investment Ratio Score (target >= 15% of income)
        if self.monthly_income > 0:
            inv_score = min((self.sip_amount / self.monthly_income) / 0.15, 1.0) * 100.0
        else:
            inv_score = 0.0

        # Net Worth Positivity (bonus/penalty)
        nw_score = 100.0 if self.net_worth >= 0 else max(0.0, 50.0 + self.net_worth / 100000 * 5)
        ...
        return round(min(max(overall, 0.0), 100.0), 2)
