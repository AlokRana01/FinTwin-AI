...
try:
    _pdf_tax = FinTwinPDFReport(
        report_title="Tax Intelligence Report",
        user_id=_uid,
        report_id=f"TX-{_uid}",
        subtitle="Old Regime vs New Regime — FY 2025-26 / AY 2026-27",
    )
    _pdf_tax.add_section("Tax Intelligence")
    ...
    _pdf_tax.add_section("Deduction Headroom", level=2)
    try:
        _pdf_tax.add_dataframe_table(headroom_df)
    except NameError:
        pass

    _pdf_tax.add_section("Personalised Tax Saving Tips", level=2)
    for i, tip in enumerate(result.get("tips", []), 1):
        _pdf_tax.add_paragraph(f"Tip {i}: {tip}")
