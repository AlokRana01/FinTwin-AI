...
st.set_page_config(page_title="Behavior Analysis", page_icon="assets/logos/favicon.svg", layout="wide")
...
with tab3:
    ...
    try:
        _pdf_report = FinTwinPDFReport(
            report_title="Spending Behavior & Peer Analysis Report",
            user_id=twin.user_id,
            report_id=f"BA-{twin.user_id}",
            subtitle="Category outflows, lifestyle-leak alerts, and peer cohort benchmarking",
        )
        _pdf_report.add_section("Expense Analysis")
        _pdf_report.add_dataframe_table(df_export)

        try:
            _pdf_report.add_section("Needs vs. Wants Breakdown", level=2)
            _pdf_report.add_plotly_figure(fig, caption="Needs vs. Wants Allocation")
        except NameError:
            pass

        try:
            _pdf_report.add_section("Peer Cohort Comparison", level=2)
