...
with tab1:
    ...
    try:
        _pdf_traj = FinTwinPDFReport(
            report_title="Financial Forecast Trajectory Report",
            user_id=twin.user_id,
            report_id=f"FC-{twin.user_id}",
            subtitle="Projected savings and net worth summary",
        )
        _pdf_traj.add_section("Forecasting Results")
        ...
        if not df_traj.empty:
            _pdf_traj.add_section("Trajectory Schedule", level=2)
...
with tab2:
    ...
    try:
        _pdf_mc = FinTwinPDFReport(
            ...
            subtitle=f"Probabilistic {mc_years}-year net worth fan path",
        )
        _pdf_mc.add_section("Scenario Simulation Results")
        _pdf_mc.add_key_value_grid({
            "Target Net Worth": target_nw,
            "Horizon (Years)": mc_years,
            "Probability of Reaching Target": f"~{prob_pct}%",
            "Median Projected Net Worth (p50)": final_month["p50"],
            "10th Percentile (Worst Case)": final_month["p10"],
            "90th Percentile (Best Case)": final_month["p90"],
        })
        _pdf_mc.add_plotly_figure(fig_mc, caption="Monte Carlo Percentile Fan Path")
        _pdf_mc.add_section("Monte Carlo Percentile Schedule", level=2)
