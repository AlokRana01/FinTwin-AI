    ...
    if planner_mode == "Single Goal Deep-Dive":
        ...
        try:
            _pdf_sg = FinTwinPDFReport(
                report_title=f"{result.goal_name} Planning Report",
                user_id=twin.user_id,
                report_id=f"GP-{twin.user_id}",
                subtitle=f"Personalized Goal Analysis — Target: ₹{result.inflation_adj_goal:,.0f}",
            )
            _pdf_sg.add_section("Goal Planning Results")
            ...
            _pdf_sg.add_section("Milestone Tracker", level=2)
    ...
    # ─────────────────────────────────────────────────────────────────────────
        ...
        try:
            _pdf_goal = FinTwinPDFReport(
                ...
                user_id=twin.user_id,
                report_id=f"GP-{twin.user_id}",
                subtitle=f"Portfolio Goal Plan — {len(results)} Active Goals",
            )
            _pdf_goal.add_section("Goal Planning Summary")
...
with tab2:
    ...
    try:
        ...
        _pdf_cmp.add_section("Cross-Goal Comparison Matrix")
