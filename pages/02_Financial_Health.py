...
try:
    _pdf_health = FinTwinPDFReport(
        report_title="Financial Health Score Report",
        user_id=twin.user_id,
        report_id=f"FH-{twin.user_id}",
        subtitle="0-100 score breakdown, benchmarks, and peer comparison",
    )
    _pdf_health.add_section("Financial Health Score")
    _pdf_health.add_key_value_grid({
        "Overall Score": f"{overall_score:.1f} / 100",
        "Financial Grade": f"Grade {financial_grade}",
        "Classification": overall_label,
    })

    _pdf_health.add_section("Scoring Factor Breakdown", level=2)
    rows = []
    for key, val in components.items():
        rows.append({
