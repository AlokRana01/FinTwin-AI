...
st.set_page_config(page_title="Financial Personality", page_icon="assets/logos/favicon.svg", layout="wide")
...
with tab2:
    ...
    try:
        _pdf_pers = FinTwinPDFReport(
            report_title="Financial Personality Report",
            user_id=twin.user_id,
            report_id=f"FP-{twin.user_id}",
            subtitle=f"Detected Archetype: {personality}",
        )
        _pdf_pers.add_section("Financial Personality")
        ...
        _pdf_pers.add_paragraph(meta.get("description", ""))
        try:
            _pdf_pers.add_plotly_figure(fig_radar, caption="Personality Ratio Radar")
        except NameError:
            pass
        if meta.get("nudges"):
            _pdf_pers.add_section("AI Financial Coach Recommendations", level=2)
            for nudge in meta["nudges"]:
                _pdf_pers.add_paragraph(f"• {nudge}")
        try:
            _pdf_pers.add_section("Cluster Statistics", level=2)
