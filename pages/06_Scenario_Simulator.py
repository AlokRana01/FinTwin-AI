    ...
    if scenario == "Normal Trajectory (Inflation Model)":
        ...
        try:
            _pdf_proj = FinTwinPDFReport(
                report_title="Inflation Model Projection Report",
                user_id=twin.user_id,
                report_id=f"SC-{twin.user_id}",
                subtitle="Normal trajectory scenario — inflation & investment growth model",
            )
            _pdf_proj.add_section("Scenario Simulation Results")
            ...
            _pdf_proj.add_section("Year-by-Year Projections", level=2)
    ...
    # ── Scenario: Job Loss Shock ─────────────────────────────────────────────
        ...
        try:
            _pdf_jl = FinTwinPDFReport(
                report_title="Job Loss Shock Report",
                user_id=twin.user_id,
                report_id=f"SC-{twin.user_id}",
                subtitle=f"Simulated {duration}-month job loss survival timeline",
            )
            _pdf_jl.add_section("Financial Risk Assessment")
            ...
            _pdf_jl.add_section("Monthly Survival Timeline", level=2)
    ...
    # ── Scenario: Home Purchase ──────────────────────────────────────────────
        ...
        try:
            _pdf_hp = FinTwinPDFReport(
                ...
                user_id=twin.user_id,
                report_id=f"SC-{twin.user_id}",
                subtitle="Property affordability & post-purchase savings trajectory",
            )
            _pdf_hp.add_section("Scenario Simulation Results")
            _pdf_hp.add_key_value_grid({
                ...
                "Can Afford Downpayment": "Yes" if result["can_afford"] else "No",
            })
            _pdf_hp.add_section("12-Month Post-Purchase Savings Trajectory", level=2)
    ...
    # ── Scenario: Inflation & Market Returns ─────────────────────────────────
        ...
        try:
            _pdf_infl = FinTwinPDFReport(
                ...
                user_id=twin.user_id,
                report_id=f"SC-{twin.user_id}",
                subtitle="Base / optimistic / pessimistic wealth scenarios",
            )
            _pdf_infl.add_section("Scenario Simulation Results")
            ...
            _pdf_infl.add_section("Scenario Data", level=2)
...
with tab2:
    ...
    try:
        _pdf_cmp = FinTwinPDFReport(
            report_title="Scenario Comparison Report",
            user_id=twin.user_id,
            report_id=f"SC-{twin.user_id}",
            subtitle=f"Before vs After — {result.scenario_name}",
        )
        _pdf_cmp.add_section("Scenario Simulation Results")
        ...
        _pdf_cmp.add_section("Full Metrics Table", level=2)
