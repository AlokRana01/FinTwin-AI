...
try:
    ...
    _pdf_coach.add_section("Executive Summary")
    _pdf_coach.add_paragraph(report.narrative)
    _pdf_coach.add_key_value_grid({
        "Overall Score": f"{report.overall_score:.0f} / 100",
        "Risk Level": report.risk_level,
        "Top Action": report.top_action,
    })

    if report.alerts:
        _pdf_coach.add_section("Risk Alerts", level=2)
        alerts_df = pd.DataFrame([
            {"Domain": a.domain.value if hasattr(a.domain, "value") else str(a.domain),
             "Severity": a.severity.value if hasattr(a.severity, "value") else str(a.severity),
             "Headline": a.headline, "Message": a.message}
            for a in report.alerts
        ])
        _pdf_coach.add_dataframe_table(alerts_df)

    if report.domain_summaries:
        _pdf_coach.add_section("Domain Summaries", level=2)
        dom_df = pd.DataFrame([
    ...
    if report.recommendations:
        _pdf_coach.add_section("Prioritised Recommendations", level=2)
        rec_df = pd.DataFrame([
