...
if not twin:
    st.markdown("""
    <div style="padding: 1.5rem 0; border-bottom: 1px solid rgba(255, 255, 255, 0.08); margin-bottom: 1.5rem;">
    ...
    if not has_financial_data:
        ...
        <div class="twin-profile-card">
            <div class="twin-profile-left">
                <div class="twin-avatar">{initials}</div>
...
<p style="color: #94A3B8; font-size: 0.85rem; margin: 0;">Export a complete copy of your Financial Digital Twin profile in structured PDF or JSON format.</p>
    ...
    try:
        _pdf_report = FinTwinPDFReport(
            ...
            subtitle="Complete Virtual Financial Self — structured sub-profile export",
        )
        _pdf_report.add_section("User Profile")
        ...
        _pdf_report.add_section("Income Profile")
        _pdf_report.add_key_value_grid(income)
        _pdf_report.add_section("Expense Profile")
        _pdf_report.add_key_value_grid(expense)
        _pdf_report.add_section("Debt Profile")
        _pdf_report.add_key_value_grid(debt)
        _pdf_report.add_section("Investment Profile")
        _pdf_report.add_key_value_grid(invest)

        if isinstance(health, dict) and health:
            _pdf_report.add_section("Financial Health Score Breakdown")
            _pdf_report.add_key_value_grid(
