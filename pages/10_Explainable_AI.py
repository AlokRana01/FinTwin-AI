...
with tab_tech:
    ...
    try:
        _pdf_xai = FinTwinPDFReport(
            report_title="Explainable AI Technical Summary",
            user_id=twin.user_id,
            report_id=f"XAI-{twin.user_id}",
            subtitle=f"{horizon}-Month Forecast & Health Score Decomposition Report",
        )
        _pdf_xai.add_section("Forecast & Explainability Summary")
        ...
        _pdf_xai.add_section("Forecast Model Contributions", level=2)
        ...
        _pdf_xai.add_section("Health Score Component Breakdown", level=2)
