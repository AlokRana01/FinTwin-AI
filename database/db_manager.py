...
class DBManager:
    ...
    def save_user_profile(user_data: Dict[str, Any]) -> bool:
        ...
        with get_db_cursor() as cursor:
            cursor.execute(
                """
                INSERT OR REPLACE INTO users (user_id, name, age, city, occupation, monthly_income, bonus, additional_income)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    uid,
    ...
    def increment_chat_usage(user_id: str) -> int:
        """Increments today's FinBot message count for this user and returns the new total."""
        import datetime
        today = datetime.date.today().isoformat()
        with get_db_cursor() as cursor:
            cursor.execute(
